...
export default function RegistrationForm({ apiBase, event, preselectCourse }: Props) {
  ...
  const fetchInventory = async () => {
    try {
      const res = await axios.get(`${apiBase}/api/event/${event.slug}/shirt-inventory`);
  ...
  const handleShirtChange = (size: string, value: string) => {
    ...
    // Check stock if inventory is loaded
    if (inventory && inventory[size] && val !== '') {
      const currentInDB = inventory[size].current;
      const limit = inventory[size].limit;
      if (currentInDB + (val as number) > limit) {
        setStockError([size.toUpperCase()]);
        return; // Don't update state if it exceeds limit
      }
    }
      ...
      {stockError && (
        <div style={{
          ...
          <div className="card glass" style={{ maxWidth: 400, textAlign: 'center', padding: 30 }}>
            <div style={{ color: '#ef4444', marginBottom: 15 }}>
              <Info size={48} />
            </div>
            <h2 style={{ color: '#fff' }}>¡Talla Agotada!</h2>
            <p style={{ color: 'rgba(255,255,255,0.8)', lineHeight: 1.5 }}>
              Lo sentimos, se han agotado las camisetas de la talla: <br />
              <strong style={{ fontSize: '1.5rem', color: '#fff' }}>{stockError.join(', ')}</strong>
            </p>
            <p style={{ fontSize: '0.9rem', marginTop: 10 }}>Por favor, selecciona otra talla disponible.</p>
            <button 
